...
C     Workspace
C
C     IWORK   INTEGER array, dimension max(M,N,NP)
C
...
C             LDWORK >= 4*N*N+2*M*M+3*M*N+M*NP+2*(N+NP)*NP+10*N+
C                       6*max(M,NP). 
...
C     .. Local Scalars ..
     ...
     $                   MINWRK, SDIM
      DOUBLE PRECISION   DEN, GAMMA, GAMMAL, GAMMAU, OMEGA, RAT,
...
C     Compute workspace.
C
      MINWRK =  4*N*N + 2*M*M + 3*M*N + M*NP + 2*( N + NP )*NP + 10*N +
     $          6*MAX( M, NP )
      IF( LDWORK.LT.MINWRK ) THEN
         INFO = -15 
      END IF
      MINCWR = ( N + M )*( N + NP ) + 3*MAX( M, NP )     
   ...
   20    CONTINUE
      ...
      ELSE
         ...
         DO 30 I = 1, N
            ...
            RAT = ABS( ( DWORK( IW3+I )/DWORK( I ) )/DEN )
...
C
     ...
     $            INFO2 )
      ...
      END IF
C
C     Compute the Hamiltonian matrix.
C
...
C
     ...
     $            DWORK( IWRK+1 ), LDWORK, BWORK, INFO2 )
      IF( INFO2.GT.0 ) THEN
         INFO = 3
         RETURN
      END IF
...
C
  ...
  310    CONTINUE
     ...
     $                CWORK( ICWRK+1 ), LCWORK-ICWRK, DWORK( IWRK+1 ),
     $                INFO2 )
         IF( INFO2.GT.0 ) THEN
            INFO = 4
            RETURN
