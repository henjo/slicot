...
C     Workspace
C
C     IWORK   INTEGER array, dimension max(2*M,2*N,N*N,NP)
C
...
C     ..
C     .. Local Scalars ..
      INTEGER            INFO2, IWC, IWD, IWRK, IWTU, IWTY, IWX, IWY,
...
C
      CALL SB10SD( N, M, NP, NCON, NMEAS, A, LDA, DWORK, N,
     $             DWORK( IWC ), NP, DWORK( IWD ), NP, AK, LDAK, BK,
     $             LDBK, CK, LDCK, DK, LDDK, DWORK( IWX ), N,
     $             DWORK( IWY ), N, RCOND( 3 ), TOLL, IWORK,
...
C
      CALL SB10TD( N, M, NP, NCON, NMEAS, DWORK( IWD ), NP,
     $             DWORK( IWTU ), M2, DWORK( IWTY ), NP2, AK, LDAK, BK,
