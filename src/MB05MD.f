...
C     of A as well as the matrix product exp(Lambda*delta) times the 
C     inverse of the eigenvector matrix of A, where Lambda is the 
...
C             matrix of eigenvalues. Note that this product is real.
C
C     LDY     INTEGER
...
C             of LDWORK, and DWORK(2) returns the reciprocal condition 
C             number of the triangular matrix used to obtain the inverse
C             of the eigenvector matrix.
C
C     LDWORK  INTEGER
...
C     Library routine DGEES is used for obtaining the right eigenvector
...
C     .. External Subroutines ..
      EXTERNAL          DGEMM, DLACPY, DSCAL, DSWAP, DTRCON, DTRMM,
     $                  DTRSM, MB05MY, XERBLA
...
C
      INFO = 0
      IF( .NOT.( LSAME( BALANC, 'N' ) .OR. LSAME( BALANC, 'S' ) ) ) THEN
         INFO = -1
      ELSE IF( N.LT.0 ) THEN
         INFO = -2
...
C     a set of linear systems, V * X = Y'.
C
      DO 40 I = 2, N
