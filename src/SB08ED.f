...
C     eigenvalues lying inside an ALPHA-stability domain.
...
C             The desired stability degree to be assigned for the
C             eigenvalues of A+H*C. The eigenvalues outside the
C             ALPHA-stability region will be assigned to have the real 
C             parts equal to ALPHA < 0 and unmodified imaginary parts
C             for a continuous-time system (DICO = 'C'), or moduli equal
C             to 0 <= ALPHA < 1 for a discrete-time system (DICO = 'D').
C
...
C
      ...
      ELSE IF( P.LT.0 ) THEN
         INFO = -4
      ELSE IF( (  DISCR .AND. ( ALPHA.LT.ZERO .OR. ALPHA.GE.ONE ) ) .OR.
     $   ( ( .NOT.DISCR .AND.   ALPHA.GE.ZERO ) ) ) THEN
         INFO = -5
...
C     prescribed stability degree ALPHA.
C
...
C
         CALL TB01XD( 'D', NQ, P, M, MIN( 1, NQ-1 ), NQ-1, A, LDA, 
