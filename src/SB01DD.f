...
C     .. Local Scalars ..
     ...
     $                   K, KK, KMR, L, LP1, M1, MI, MP1, MR, MR1,
     $                   NBLKCR, NC, NI, NJ, NP1, NR, NR1, RANK
...
C
   ...
   10 CONTINUE     
      ...
      ELSE IF( LDG.LT.MAX( 1, M ) ) THEN
         INFO = -16
      ELSE IF( LDWORK.LT.MAX( M*N, M*M + 2*N + 4*M ) ) THEN
         INFO = -20
      END IF
C
      IF( INFO.NE.0 ) THEN
