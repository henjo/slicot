...
C             leading N-by-P part of this array contains AK , the
C                                                          i     
C             product of the input matrix A and the Kalman filter gain 
C             matrix at instant i.
C
C     LDK     INTEGER
...
C     .. Local Scalars ..
      ...
      INTEGER           I, I12, II, ITAU, JWORK, LDW, N1, PL, PN, WRKOPT
...
C
      ...
      ELSE IF( LDB.LT.N1 ) THEN
         INFO = -11
      ELSE IF( LMULTB .AND. LDQ.LT.1 .OR.
     $    .NOT.LMULTB .AND. LDQ.LT.MAX( 1, M ) ) THEN
         INFO = -13
...
C     fully exploited. Specifically, the following partition is used:
C
C        [ C1  0  ] [ S1  0  ]
C        [ A1  A3 ] [ S2  S3 ],
C        [ A2  A4 ] 
...
C     matrix above is stored in the workspace.
C
...
C
      LDW = MAX( 1, PN )
      CALL DLACPY( 'Lower', P, P, C, LDC, DWORK, LDW )
      CALL DLACPY( 'Full',  N, P, A, LDA, DWORK(P+1), LDW )
      CALL DLACPY( 'Lower', N, N-P, A(1,P+1), LDA, DWORK(P*LDW+P+1),
     $             LDW )
C
C             [ C1  0  ] 
C     Compute [        ] x S as a product of lower triangular matrices. 
C             [ A1  A3 ]             
...
C
     ...
     $            ONE, S, LDS, DWORK(N+1), LDW )
C
C     Triangularization (2 steps). 
C
C     Step 1: annihilate the matrix C x S = C1 x S1.
C     Workspace: need (N+P)*N + 2*P.
C     
      ITAU  = LDW*N + 1
      JWORK = ITAU + P
C
      CALL MB04LD( 'Lower', P, N, N, R, LDR, DWORK, LDW, DWORK(P+1),
     $             LDW, K, LDK, DWORK(ITAU), DWORK(JWORK) )
...
C
      CALL DLACPY( 'Full', N, N, DWORK(P+1), LDW, DWORK, N1 )
C
      DO 20 I = N*(N-1) + 1, 1, -N 
...
C     
      CALL DLACPY( 'Full', N, M, B, LDB, DWORK, LDW )
      IF ( .NOT.LMULTB )
...
C     
      ...
      JWORK = ITAU + N
C
      CALL MB04JD( N, M+N, N-P-1, 0, DWORK, LDW, DWORK, LDW, 
...
C
      CALL DLACPY( 'Lower', N, N, DWORK, LDW, S, LDS )
C
      IF ( LJOBK ) THEN
C
C        Compute K.
