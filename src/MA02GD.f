...
C             If IPIV is negative, the interchanges are applied in 
C             reverse order.
C
C     METHOD
C
...
C     B are N-by-N and N-by-M, respectively, the following statements
C     can be used:
C
C         CALL DGETRF( N, N, A, LDA, IPIV, INFO )
C         CALL DTRSM( 'R', 'U', 'N', 'N', N, M, ONE, A, LDA, B, LDB )
C         CALL DTRSM( 'R', 'L', 'N', 'U', N, M, ONE, A, LDA, B, LDB )
C         CALL MA02GD( N, B, LDB, 1, N, IPIV, -1 )
C
C     CONTRIBUTOR
C
