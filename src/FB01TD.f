...
C             (hence the information square root) at instant i.
C             The strict lower triangular part of this array is not
C             referenced.
C
C     LDQINV  INTEGER
...
C
      ...
      ELSE IF( LDAINB.LT.N1 ) THEN
         INFO = -11
      ELSE IF( LMULTR .AND. LDRINV.LT.1 .OR.
     $    .NOT.LMULTR .AND. LDRINV.LT.MAX( 1, P ) ) THEN
         INFO = -13
...
C     [ AINVB AINV ] is fully exploited. Specifically, the following
C     partition is used:
C
C       [ S1  S2 ] [ B1  A1 A3 ] 
C       [ 0   S3 ] [ 0   A2 A4 ],
...
C
      ...
      CALL DLACPY( 'Full',  M, N, AINV, LDAINV, DWORK(I32), LDW )
      CALL DLACPY( 'Upper', N-M, N, AINV(MP1,1), LDAINV, DWORK(I32+M),
     $             LDW )
C
C                 [ B1  A1 ] 
C     Compute S x [ 0   A2 ] as a product of upper triangular matrices. 
...
C
      DO 20 I = 1, M
         CALL DAXPY( I, -DWORK(ITAU+I-1)*( DWORK(I13+I-1) +
     $               DDOT( I, DWORK(IJ), 1, X, 1 ) ), DWORK(IJ), 1, X, 
     $               1 )
         IJ = IJ + LDW
   20 CONTINUE
C
...
C
      CALL DLACPY( 'Full', M, N, DWORK(I32), LDW, DWORK, LDW )
      CALL DLACPY( 'Upper', N-M, N, DWORK(I32+M), LDW, DWORK(MP1), LDW )
      LDW = MAX( 1, NP )
C
      DO 30 I = N, 1, -1
         CALL DCOPY( MIN( N, I+M ), DWORK(N1*(I-1)+1), -1,
     $               DWORK(LDW*(I-1)+P+1), -1 )
   30 CONTINUE
C
...
C     
      ...
      JWORK = ITAU + N
C
      CALL MB04ID( NP, N, N-MP1, 1, DWORK, LDW, DWORK(I23), LDW, 
     $             DWORK(ITAU), DWORK(JWORK), LDWORK-JWORK+1, INFO )
      WRKOPT = MAX( WRKOPT, INT( DWORK(JWORK) )+JWORK-1 )
C
C     Output SINV, E  and X (if needed) and set the optimal workspace 
