...
C             INUL(i) = .TRUE., are the (N+L-RANK) base vectors of the
C             right singular subspace corresponding to the singular
C             values of C = [A|B] which are less than or equal to THETA.
C             The TLS solution is computed from these vectors.
C             The other columns contain no useful information.
C
C     LDC     INTEGER
...
C                          min(M,N+L) + LW + max(6*(N+L)-5,L*L+N+L) 
C                                     + 3*L),
C             where
...
C
      ...
      ELSE IF( LDX.LT.MAX( 1, N ) ) THEN
         INFO = -9
      ELSE IF( LDWORK.LT.MAX( 2, K + 2*P, JV + 3*L ) ) THEN
         INFO = -16
      END IF
C
      IF ( INFO.NE.0 ) THEN
...
C
      IF ( M.EQ.0 .OR. N.EQ.0 .OR. L.EQ.0 ) THEN
         RANK = 0
         DWORK(1) = TWO
         DWORK(2) = ONE
         RETURN
...
C     
      IF ( M.GE.ILAENV( 6, 'DGESVD', 'N' // 'N', M, NL, 0, 0 ) ) THEN
C
...
C
      ...
      MC = NL - IOFF
      KF = IHOUSH + P*NL
      SUFWRK = LDWORK.GE.( KF + NL**2 + MAX( NL, 3*L ) - 1 )
      IF ( SUFWRK ) THEN
C
...
C
  100 CONTINUE
     ...
     $             LDWORK-JWORK+1, IWARN, INFO )
      WRKOPT = MAX( WRKOPT, JWORK + 6*NL - 5 )
C
...
C
C        Not enough workspace for a fast algorithm or subsequent passes.
C        Workspace: LDW2, where LDW2 = LDW + max(6*(N+L)-5,L*L+N+L).
C
         WRKOPT = JV + MAX( 6*NL - 5, L*L + NL ) - 1
         I  = NL
         JF = JV
         LDF = L
         JWORK = JF + LDF*L
...
C
               DO 360 NJ = NL, 1, -1
                  CALL DCOPY( P, DWORK(MJ), -1, DWORK(KJ), -1 )
                  KJ = KJ - NL
                  MJ = MJ - P
  360          CONTINUE
C
...
C
            DO 380 NJ = 1, MIN( P, MC )
               J = MC - NJ + 1
               CALL DCOPY( J, DWORK(MJ), P, DWORK(KJ), 1 )
               KJ = KJ + J
               MJ = MJ + NL + 1
  380       CONTINUE
C
