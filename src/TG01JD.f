...
C             or block Hessenebrg, in accordance to the last
...
C             On exit, if INFO = 0, the leading INDRED(7) elements of
...
C     Phase 1: Eliminate finite uncontrolable eigenvalues.
...
C              SYSTYP = 'R' or 'S'.
C     Phase 2: Eliminate infinite and finite nonzero uncontrollable
C              eigenvalues. The resulting matrix ( Br Er ) is in a
C              controllable staircase form (see TG01HD), and Ar is
C              upper triangular.
C              This phase is performed if JOB = 'I' or 'C' and
C              SYSTYP = 'R' or 'P'.
C     Phase 3: Eliminate finite unobservable eigenvalues.
...
C              SYSTYP = 'R' or 'S'.
C     Phase 4: Eliminate infinite and finite nonzero unobservable
...
C     .. Local Scalars ..
     ...
     $                  LJOBIR, LJOBO, LSPACE, LSYSP, LSYSR, LSYSS
      INTEGER           KWA, KWB, KWC, KWE, LBA, LBE, LDQ, LDZ, MAXMP, 
     $                  NBLCK, NC
C     .. Local Arrays ..
...
C
      ...
      ELSE IF( LDB.LT.MAX( 1, N ) ) THEN
         INFO = -12
      ELSE IF( ( N.GT.0 .AND. LDC.LT.MAX( 1, MAXMP ) ) .OR.
...
C
     ...
     $                C, LDC, DWORK(1), DWORK(N+1), DWORK(2*N+1), INFO )
      END IF
C
      JOBQ = 'N'
      JOBZ = 'N'
...
C
            CALL DLACPY( 'Full', NC, NC, A, LDA, DWORK(KWA), N )
            CALL DLACPY( 'Full', NC, NC, E, LDE, DWORK(KWE), N )
            CALL DLACPY( 'Full', NC, M,  B, LDB, DWORK(KWB), N )
            CALL DLACPY( 'Full', P,  NC, C, LDC, DWORK(KWC), P )
         END IF
C
...
C
     ...
     $                E, LDE, B, LDB, C, LDC, DUM, LDQ, DUM, LDZ, NR, 
...
C
            CALL DLACPY( 'Full', NC, NC, DWORK(KWA), N, A, LDA )
            CALL DLACPY( 'Full', NC, NC, DWORK(KWE), N, E, LDE )
            CALL DLACPY( 'Full', NC, M,  DWORK(KWB), N, B, LDB )
            CALL DLACPY( 'Full', P,  NC, DWORK(KWC), P, C, LDC )
         END IF
      END IF
C
      IF( INFCON ) THEN
C
C        Phase 2: Eliminate infinite and finite nonzero uncontrolable 
C                 eigenvalues.
C
         IF( LSPACE ) THEN
C
...
C
     ...
     $                A, LDA, B, LDB, C, LDC, DUM, LDQ, DUM, LDZ, NR, 
     $                NBLCK, IWORK, TOL, IWORK(N+1), DWORK, INFO )
         IF( NR.LT.NC .OR. .NOT.LSPACE ) THEN
            IF( NBLCK.GT.1 ) THEN
...
C
            CALL DLACPY( 'Full', NC, NC, A, LDA, DWORK(KWA), N )
            CALL DLACPY( 'Full', NC, NC, E, LDE, DWORK(KWE), N )
            CALL DLACPY( 'Full', NC, P,  B, LDB, DWORK(KWC), N )
            CALL DLACPY( 'Full', M,  NC, C, LDC, DWORK(KWB), M )
         END IF
C
...
C
     ...
     $                E, LDE, B, LDB, C, LDC, DUM, LDZ, DUM, LDQ, NR, 
     $                NBLCK, IWORK, TOL, IWORK(N+1), DWORK, INFO )
         IF( NR.LT.NC .OR. .NOT.LSPACE ) THEN
            IF( NBLCK.GT.1 ) THEN
               LBA = IWORK(1) + IWORK(2) - 1 
...
C
            CALL DLACPY( 'Full', NC, NC, DWORK(KWA), N, A, LDA )
            CALL DLACPY( 'Full', NC, NC, DWORK(KWE), N, E, LDE )
            CALL DLACPY( 'Full', NC, P,  DWORK(KWC), N, B, LDB )
            CALL DLACPY( 'Full', M,  NC, DWORK(KWB), M, C, LDC )
         END IF
      END IF
C
      IF( INFOBS ) THEN
...
C
     ...
     $                A, LDA, B, LDB, C, LDC, DUM, LDZ, DUM, LDQ, NR, 
     $                NBLCK, IWORK, TOL, IWORK(N+1), DWORK, INFO )
         IF( NR.LT.NC .OR. .NOT.LSPACE ) THEN
            IF( NBLCK.GT.1 ) THEN
               LBE = IWORK(1) + IWORK(2) - 1 
