     ...
     $                   B, LDB, Y, JPVT, RANK, SVAL, DWORK, INFO )
C
...
C             the elements below the diagonal, with the first min(M,N) 
C             entries of the array DWORK, represent the orthogonal 
...
C             (max( min(M,N)+3*N, 2*min(M,N)+NRHS ))
C             The first min(M,N) + RANK entries contain the scalar 
C             factors of the elementary reflectors used in the complete
C             orthogonal factorization of A.
C             Among the first min(M,N) entries, only the first RANK
...
C     METHOD
C
C     If INIPIV = 'P', the routine first computes a QR factorization 
...
C     If INIPIV = 'N', the effective rank is estimated during a 
...
C     .. External Subroutines ..
      EXTERNAL           DLABAD, DLACPY, DLASCL, DLASET, DLATZM, DORM2R,
     $                   DTRSM, DTZRQF, MB03OD, MB03OY, XERBLA
C     ..
...
C        A * P = Q * R.
C     Workspace min(M,N)+3*N. Details of Householder transformations
C     stored in DWORK(1:MN).
C
      IF( PERMUT ) THEN
         CALL MB03OD( 'Q', M, N, A, LDA, JPVT, RCOND, SVLMAX, 
     $                DWORK( 1 ), RANK, SVAL, DWORK( MN+1 ), INFO )
      ELSE
...
C
      IF( RANK.LT.N )
     $   CALL DTZRQF( RANK, N, A, LDA, DWORK( MN+1 ), INFO )
C
      IF( NRHS.GT.0 ) THEN
C
...
C        
     ...
     $                DWORK( 1 ), B, LDB, DWORK( 2*MN+1 ), INFO )
C        
