...
C     .. Local Scalars ..
      LOGICAL           LOVER, LUPLO
      INTEGER           I1, I2, J, LDWN2, LDWP1, LDWP2
C     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
C     .. External Subroutines ..
      EXTERNAL          DCOPY, DGEMM, DLACPY, DLASET, XERBLA
...
C
               DO 20 J = N2, 1, -1
                  CALL DCOPY( N2, A2(1,J), -1, A(1,J), -1 )
   20          CONTINUE
C
            END IF
         ELSE
...
C
      ...
      CALL DGEMM ( 'No transpose', 'No transpose', N2, N1, P1, ONE, B2,
     $             LDB2, C1, LDC1, ZERO, A(I2,I1), LDA )
C
      IF ( LUPLO ) THEN
C     
C        Form the matrix  B.
...
C
               DO 30 J = M1, 1, -1
                  CALL DCOPY( N1, B1(1,J), -1, B(1,J), -1 )
   30          CONTINUE
C
            END IF
         ELSE
...
C
     ...
     $                B2, LDB2, D1, LDD1, ZERO, B(I2,1), LDB )
C     
C        Form the matrix  C.
C     
         IF ( LOVER ) THEN
...
C
               DO 40 J = N2, 1, -1
                  CALL DCOPY( P2, C2(1,J), -1, C(1,J), -1 )
   40          CONTINUE
C
            END IF
         ELSE
         ...
         END IF
C
         CALL DGEMM ( 'No transpose', 'No transpose', P2, N1, P1, ONE, 
     $                D2, LDD2, C1, LDC1, ZERO, C(I1,1), LDC )
C     
C        Now form the matrix  D.
C     
         IF ( LOVER ) THEN
