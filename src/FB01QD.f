...
C     .. Local Scalars ..
      ...
      INTEGER           I12, ITAU, JWORK, LDW, N1, PN, WRKOPT
...
C
      LDW = MAX( 1, PN )
      CALL DLACPY( 'Full', N, N, A, LDA, DWORK, LDW )
...
C     
      ...
      JWORK = ITAU + P
C
      CALL MB04LD( 'Full', P, N, N, R, LDR, DWORK(N+1), LDW, DWORK, LDW,
     $             K, LDK, DWORK(ITAU), DWORK(JWORK) )
      WRKOPT = MAX( 1, PN*N + 2*P )
C
...
C
      CALL DLACPY( 'Full', N, N, DWORK, LDW, DWORK, N1 )
      LDW = N1
      I12 = LDW*N + 1
C
...
C     
      CALL DLACPY( 'Full', N, M, B, LDB, DWORK(I12), LDW )
      IF ( .NOT.LMULTB )
     $   CALL DTRMM( 'Right', 'Lower', 'No transpose', 'Non-unit', N, M,
     $               ONE, Q, LDQ, DWORK(I12), LDW )
...
C     
      ...
      JWORK = ITAU + N
C
      CALL DGELQF( N, N+M, DWORK, LDW, DWORK(ITAU), DWORK(JWORK),
     $             LDWORK-JWORK+1, INFO )
