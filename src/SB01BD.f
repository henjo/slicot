...
C     .. External Subroutines ..
      EXTERNAL         DCOPY, DGEES, DGEMM, DLAEXC, DLASET, DROT, DSWAP,
...
C
         ...
         DO 10 I = 1, NP
            IF( WI(I) .EQ. ZERO ) THEN
               ...
               IF( K .GT. 0 ) THEN
                  S = WR(I)
                  CALL DCOPY( K, WR(NPR), -1, WR(NPR+1), -1 )
                  CALL DCOPY( K, WI(NPR), -1, WI(NPR+1), -1 )
                  WR(NPR) = S
                  WI(NPR) = ZERO
               END IF
            END IF
...
C                    in Z.
C
                     CALL DROT( N-NL+1, A(NL,NL), LDA, A(NSUP,NL), LDA, 
     $                          C, S )
