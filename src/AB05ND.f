...
C             LDWORK >= MAX(1, P1*P1, M1*M1, N1*P1).
C
C     Error Indicator
C
C     INFO    INTEGER
...
C     .. Local Scalars ..
      LOGICAL           LOVER
      INTEGER           J, LDWM1, LDWN1, LDWP1
C     .. External Functions ..
      LOGICAL           LSAME
      EXTERNAL          LSAME
C     .. External Subroutines ..
      EXTERNAL          DGEMM, DGETRF, DGETRS, DLACPY, DLASET, XERBLA
...
C
     ...
     $         ( N1.EQ.0 .AND. LDC1.LT.1 ) ) THEN
      ...
      ELSE IF( LDB2.LT.MAX( 1, N2 ) ) THEN
         INFO = -18
      ELSE IF( ( N2.GT.0 .AND. LDC2.LT.MAX( 1, M1 ) ) .OR.
     $         ( N2.EQ.0 .AND. LDC2.LT.1 ) ) THEN
         INFO = -20
      ELSE IF( LDD2.LT.MAX( 1, M1 ) ) THEN
         INFO = -22
     ...
     $         ( N.EQ.0 .AND. LDC.LT.1 ) ) THEN
      ...
      ELSE IF( LDD.LT.MAX( 1, P1 ) ) THEN
         INFO = -31
      ELSE IF( LDWORK.LT.MAX( 1, P1*P1, M1*M1, N1*P1 ) ) THEN
         INFO = -34
      END IF
C
      IF ( INFO.NE.0 ) THEN
...
C     
      ...
      LDWN1 = MAX( 1, N1 )
C
      CALL DLASET( 'F', P1, P1, ZERO, ONE, DWORK, LDWP1 )
      CALL DGEMM ( 'No transpose', 'No transpose', P1, P1, M1, ALPHA,
     $             D1, LDD1, D2, LDD2, ONE, DWORK, LDWP1 )
C     
C     Factorize this matrix.
C     
      CALL DGETRF( P1, P1, DWORK, LDWP1, IWORK, INFO )
C
      IF ( INFO.EQ.0 ) THEN
C     
C        Form  E21 * D1.
...
C
               DO 10 J = M1, 1, -1
                  CALL DCOPY( P1, D1(1,J), -1, D(1,J), -1 )
   10          CONTINUE
C
            END IF
         ELSE
         ...
         END IF
C
         CALL DGETRS( 'No transpose', P1, M1, DWORK, LDWP1, IWORK, D,
     $                LDD, INFO )
C     
C        Form  E21 * C1.
C     
...
C
               DO 20 J = N1, 1, -1
                  CALL DCOPY( P1, C1(1,J), -1, C(1,J), -1 )
   20          CONTINUE
C
            END IF
         ELSE
         ...
         END IF
C
         CALL DGETRS( 'No transpose', P1, N1, DWORK, LDWP1, IWORK, C,
     $                LDC, INFO )
C     
...
C
            IF ( N1*M1.LT.N*N2 ) THEN
C
C              Use BLAS 3 code.
C
               CALL DLACPY( 'F', N1, M1, B1, LDB1, A(1,N1+1), LDA )
               CALL DGEMM ( 'No transpose', 'No transpose', N1, M1, M1,
     $                      ONE, A(1,N1+1), LDA, DWORK, LDWM1, ZERO, B,
     $                      LDB )
            ELSE
C
C              Use BLAS 2 code.
...
C     
         CALL DGEMM ( 'No transpose', 'No transpose', N2, M1, P1, ONE,
     $                B2, LDB2, D, LDD, ZERO, B(N1+1,1), LDB )
...
C     
     ...
     $                -ALPHA, B, LDB, D2, LDD2, ZERO, DWORK, LDWN1 )
C
         IF ( LOVER .AND. LDA1.LE.LDA ) THEN
            IF ( LDA1.LT.LDA ) THEN
C
               DO 40 J = N1, 1, -1
                  CALL DCOPY( N1, A1(1,J), -1, A(1,J), -1 )
   40          CONTINUE
C
            END IF
         ELSE
         ...
         END IF
C
         CALL DGEMM ( 'No transpose', 'No transpose', N1, N1, P1, ONE,
     $                DWORK, LDWN1, C1, LDC1, ONE, A, LDA )
C     
...
C     
     ...
     $                B2, LDB2, C, LDC, ZERO, A(N1+1,1), LDA )
