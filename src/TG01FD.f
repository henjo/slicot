...
C             LDWORK >= MAX( 1, P, MIN(L,N)+MAX(3*N,M,L) ).
C
C     Error Indicator
C
C     INFO    INTEGER
...
C     If JOBA = 'T' then A22 is row compressed using the QR factorization
C     with column pivoting to the form
C
C                  ( Ar  X )
C            A22 = (       ) 
C                  (  0  0 )
...
C     .. Array Arguments ..
     ...
     $                   Z( LDZ, * )
C     .. Local Scalars ..
      LOGICAL            ILQ, ILZ, REDA, REDTR
      INTEGER            I, ICOMPQ, ICOMPZ, IR1, IRE1, J, K, KK, KW, 
     $                   LA22, LH, LN, LWR, NA22, WRKOPT
      DOUBLE PRECISION   SVLMAX, T, TOLDEF
C     .. Local Arrays ..
...
C     .. External Subroutines ..
      EXTERNAL           DLARFG, DLASET, DLATZM, DORMQR, DSWAP, MB03OY, 
     $                   XERBLA
...
C
      INFO = 0
      WRKOPT = MAX( 1, P, MIN( L, N ) + MAX( 3*N, M, L ) )
      IF( ICOMPQ.LE.0 ) THEN
         INFO = -1
...
C
         IF( RANKE.LT.N ) THEN
            ...
            IR1 = RANKE + 1
            DO 40 K = RANKE, 1, -1
               CALL DLARFG( LH, E(K,K), E(K,IR1), LDE, T )
               CALL DLATZM( 'Right', K-1, LH, E(K,IR1), LDE, T,
     $                      E(1,K), E(1,IR1), LDE, DWORK )
               CALL DLATZM( 'Right', L, LH, E(K,IR1), LDE, T,
     $                      A(1,K), A(1,IR1), LDA, DWORK )
               CALL DLATZM( 'Right', P, LH, E(K,IR1), LDE, T,
     $                      C(1,K), C(1,IR1), LDC, DWORK )
               IF( ILZ )
     $            CALL DLATZM( 'Right', N, LH, E(K,IR1), LDE, T,
     $                         Z(1,K), Z(1,IR1), LDZ, DWORK )
   40       CONTINUE
C
C           Set E12 and E22 to zero.
C
            CALL DLASET( 'Full', L, LH-1, ZERO, ZERO, E(1,IR1), LDE )
         END IF
      ELSE
...
C
                  ...
                  DO 80 K = RNKA22, 1, -1
                     KK = RANKE + K
                     CALL DLARFG( LH, A(KK,KK), A(KK,IRE1), LDA, T )
                     CALL DLATZM( 'Right', KK-1, LH, A(KK,IRE1), LDA, T,
     $                            A(1,KK), A(1,IRE1), LDA, DWORK )
                     CALL DLATZM( 'Right', P, LH, A(KK,IRE1), LDA, T,
     $                            C(1,KK), C(1,IRE1), LDC, DWORK )
                     IF( ILZ )
     $                  CALL DLATZM( 'Right', N, LH, A(KK,IRE1), LDA, T,
     $                               Z(1,KK), Z(1,IRE1), LDZ, DWORK )
   80             CONTINUE
C
C                 Set R12 and R22 to zero.
C
                  CALL DLASET( 'Full', LA22, LH-1, ZERO, ZERO, 
     $                         A(IR1,IRE1), LDA )
               END IF
            ELSE
