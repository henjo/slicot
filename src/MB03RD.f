...
C             If SORT = 'N', this parameter is not referenced.
C
C     Workspace
C
...
C     A  , whose eigenvalue(s) is (are) the closest to those of A   is
C      22                                                        11
C     selected, and moved by orthogonal similarity transformations in
C     the leading position of A  ; the moved diagonal block is then
C                              22
C     added to the block A  , increasing its order by 1 (or 2). Another
C                         11
C     attempt is made to compute a suitable transformation matrix X with
C     the new definitions of the blocks A   and A  . After a successful
C                                        11      22
C     transformation matrix X has been obtained, it postmultiplies the
C     current transformation matrix (if JOBX = 'U'), and the whole
C     procedure is repeated for the matrix A  .
C                                           22
C
...
C
      ...
      IF( .NOT.LJOBX .AND. .NOT.LSAME( JOBX, 'N' ) ) THEN
         INFO = -1
      ELSE IF( .NOT.LSORT .AND. .NOT.LSAME( SORT, 'N' ) ) THEN
         INFO = -2
      ELSE IF( N.LT.0 ) THEN
         INFO = -3
...
C              nearest eigenvalues to the mean of eigenvalues of A11 and
C              resume the loop.
...
C
               ...
               CAV = CAV/DA11
C
C              Loop to find the eigenvalue of A22 nearest to the above
C              computed mean.
C
