...
C     .. External Subroutines ..
     ...
     $                  DLASET, DSCAL, MB04MD, MB05OY, XERBLA
C     .. Intrinsic Functions ..
      INTRINSIC         DBLE, EXP, INT, LOG, LOG10, MAX, MOD, SQRT
...
C
      ...
      ELSE IF( LDA.LT.MAX( 1, N ) ) THEN
         INFO = -6
      ELSE IF( ( N.LE.1 .AND. LDWORK.LT.1 ) .OR.
     $         ( N.GT.1 .AND. LDWORK.LT.N*( 2*N + NDIAG + 1 ) + NDIAG ) 
     $       ) THEN
         INFO = -11
      END IF
C
...
C     so that the resulted matrix has the 1-norm less than 1.
C
      ANORM = DLANGE( '1-norm', N, N, A, LDA, DWORK(JWORA1) )
      M = 0
      IF ( ANORM.GT.ONE ) THEN
         MPOWER = INT( LOG( ONE/EPS )/LOG( TWO ) )
         FACTOR = TWO
         DO 80 I = 1, MPOWER
            M = I
            IF ( ANORM.LE.FACTOR )
...
C
      ...
      IF ( M.NE.0 ) THEN
         ...
         GN = ( TWO*( XN**2 ) + TEN*XN - FOUR )/( ( ( XN + TWO )**2 )
     $                                           *( ( XN + ONE )**2 ) )
C
...
C
     ...
     $                        ANORM ) THEN  
               EABS = XN*EPS*TEMP + TWO*EABS*ANORM + EABS**2
            ELSE
               EABS = OVRTHR
            END IF
C
...
C
      SD2 = TWO*SUM2D*SQRT( VAR + ( EIGHT*XN*( ANORM**2 )*VAREPS ) )/XN
      IF ( LBALS ) THEN
         SIZE = ZERO
      ELSE
         SIZE = EMNORM + SD2
...
C
     ...
     $       LOG10( EMNORM ) - LOG10( EPS )
      ...
      MDIG = NDEC - INT( RERR + HALF )
      IDIG = NDEC - INT( RERL + HALF )
