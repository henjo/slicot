...
C  
     ...
     $                      A(IR1,IR1), LDA, DWORK(IR1), B(IR1,1), LDB, 
     $                      DWORK(KW), LWR, INFO )
               WRKOPT = MAX( WRKOPT, LN + INT( DWORK(KW) ) ) 
C
C              Q <-- Q * diag(I, Q2)
...
C                 C <-- C * diag(I,Zr2)
C
                  IF( LWR.GT.P*LN2 ) THEN
C
                     CALL DGEMM( 'No transpose', 'Transpose', P, LN2, 
     $                           LN2, ONE, C(1,IR1), LDC, A(IR1,IR1), 
     $                           LDA, ZERO, DWORK(KW), MAX( 1, P ) )
                     CALL DLACPY( 'Full', P, LN2, DWORK( KW ),
     $                            MAX( 1, P ), C(1,IR1), LDC )
                  ELSE
                     DO 80 I = 1, P
...
C  
     ...
     $                      A(IR1,IR1), LDA, DWORK(IR1), C(1,IR1), LDC, 
     $                      DWORK(KW), LWR, INFO )
               WRKOPT = MAX( WRKOPT, LN + INT( DWORK(KW) ) ) 
C
C              Z' <-  diag(I, Z2) * Z'
