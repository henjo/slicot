...
C             Specifies the balancing operations (permuting and/or
C             scaling) to be performed by the LAPACK subroutine DGEBAL
C             on the Hessenberg matrix A'' in (2), as follows:
C             = 'N':  do neither permuting nor scaling;
C             = 'P':  attempt to permute the matrix A'' in (2) to block
C                     triangular form;
C             = 'S':  do scaling in order to equilibrate the rows
C                     and columns of A'';
C             = 'B':  perform both, 'P' and 'S'.
...
C     .. Local Scalars ..
      ...
      INTEGER           I, IGNORE, IHI, ILO, J, JWORK, M, N2
      LOGICAL           SORTED
C     ..
C     .. Local Arrays ..
