...
C     .. Local Scalars ..
     ...
     $                  BIGNUM, CI, CR, EPS, L, LAMI, LAMR, SCALE1, SI,
     $                  SMLNUM, SR, T, V, W, XR, XI, YR, YI
...
C     .. External Subroutines ..
      EXTERNAL          DCOPY, DGEMM, DGEMV, DLABAD, DLADIV, SG03BY
...
C
     ...
     $                  ABS( EE(2,2) ) ), SMLNUM )
      ...
      END IF
      W = EE(1,1)*EE(2,2)
      V = ( AA(2,1)*EE(1,2) - AA(2,2)*EE(1,1) - AA(1,1)*EE(2,2) )/W
      W = ( AA(1,1)*AA(2,2) - AA(2,1)*AA(1,2) )/W
      LAMR = -V/TWO
      LAMI = LAMR**2 - W
      IF (LAMI .GT. ZERO) THEN
         INFO = 2
         RETURN
      END IF
      LAMI = SQRT( -LAMI )
...
C
      CALL SG03BY( AA(1,1) - EE(1,1)*LAMR, -EE(1,1)*LAMI, AA(2,1),
     $             ZERO, CR, CI, SR, SI, L )
      QR(1,1) =  CR
      QR(1,2) =  SR
      QR(2,1) = -SR
