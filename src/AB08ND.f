...
C     state-space model (A,B,C,D) a regular pencil (lambda*B - A ) which
C                                                           f   f
...
C                            MIN(M+1,P) + MAX(3*P,N),
C                            MIN(M+1,N) + MAX(3*(M+1),N+P) ).
C             An upper bound is MAX(s+1,N) + MAX(3*(s+1),N+s), with
C             s = MAX(M,P).
...
C     system (A,B,C,D) a regular pencil lambda*B - A  which has the
C                                               f   f
...
C            (lambda*I - A  B) in order to reduce it to
C            (     -C       D)
C
C            (lambda*B  - A   X)
...
C     Library routine DGEGV with A = A , B = B  and N = NU.
C                                     f       f
C     CONTRIBUTOR
C
...
C     .. Local Scalars ..
      LOGICAL           LEQUIL
      INTEGER           I, I0, I1, II, J, MM, MNU, MU, NINFZ, NN, NU1,
     $                  NUMU, NUMU1, PP, RO, SIGMA, WRKOPT
      DOUBLE PRECISION  MAXRED, S, SVLMAX, THRESH, TOLER
C     .. External Functions ..
      LOGICAL           LSAME
      DOUBLE PRECISION  DLAMCH, DLANGE
      EXTERNAL          DLAMCH, DLANGE, LSAME
C     .. External Subroutines ..
      EXTERNAL          AB08NX, DCOPY, DLACPY, DLARFG, DLASET, DLATZM,
     $                  TB01ID, XERBLA
...
C
      ...
      ELSE IF( LDD.LT.MAX( 1, P ) ) THEN
         INFO = -12
      ELSE IF( LDAF.LT.MAX( 1, N + MAX( M, P ) ) ) THEN
         INFO = -22
...
C     Workspace: need   MAX( MIN(P+1,M) + MAX(3*M,N),
C                            MIN(P+1,N) + MAX(3*(P+1),N+M) );
C                prefer larger.
C
      RO = PP
...
C           with D square invertible.
C           Workspace: need  MAX( MIN(M+1,P) + MAX(3*P,N),
C                                 MIN(M+1,N) + MAX(3*(M+1),N+P) );
C                prefer larger.
C
            RO = MAX( PP - MM, 0 )
...
C           Workspace: need   N + MIN(M,P) - 1.
C
            CALL DLASET( 'Full', NU, MM, ZERO, ZERO, BF, LDBF )
            CALL DLASET( 'Full', NU, NU, ZERO, ONE,  BF(1,1+MM), LDBF )
C
            IF ( RANK.NE.0 ) THEN
               NU1 = NU + 1
               I1 = NU + MU
C
               DO 100 I0 = MM, 1, -1
                  CALL DLARFG( NU1, AF(I1,I0+NU), AF(I1,I0), LDAF, S )
                  CALL DLATZM( 'Right', I1-1, NU1, AF(I1,I0), LDAF, S,
     $                         AF(1,I0+NU), AF(1,I0), LDAF, DWORK )
                  CALL DLATZM( 'Right', NU, NU1, AF(I1,I0), LDAF, S,
     $                         BF(1,I0+NU), BF(1,I0), LDBF, DWORK )
                  I1 = I1 - 1
  100          CONTINUE
C
            END IF
