...
C     .. External Subroutines ..
      EXTERNAL          AB04MD, AB09AX, DAXPY, DCOPY, DGELSX, DGEMM,
...
C     Workspace: N*(M+P+2) + 2*M*P +
C                max(min(KR,M)+3*M,2*min(KR,M)+P).
C
      DO 55 J = 1, M
         IWORK(J) = 0
   55 CONTINUE
      CALL DGELSX( KR, M, P, DWORK(KB2), LDB2, DWORK(KC2T), LDC2T,
     $             IWORK, RTOL, IRANK, DWORK(KW), IERR )
